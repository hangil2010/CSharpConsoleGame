...
{
    // 과제 1. 콘솔 프로젝트 제작
    // C# 기초 문법을 활용해 콘솔 환경의 텍스트 게임을 제작하시오.
    // 1. 2차원 배열과 Console ReadKey를 통해 캐릭터 이동 구현하기
    // 2. Console Clear와 같은 기능으로 이동 후 새로 그리는 개념
    // 3. 2차원 배열에 들은 요소에 따라 이동 가능, 불가능 판단
    // 4. 게임적인 메커니즘을 추가하여 목표를 달성하도록 함
    // 개인적 매커니즘 : 미로를 지나가며 아이템을 먹어야 한다. 한번 왔던 길은 다시 못가게 한다.
    // 추가로 해야 할 기능 : 게임 실패 시 다시 시작할 수 있게 한다.
